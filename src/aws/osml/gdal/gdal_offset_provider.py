...
class GDALOffsetProvider(ElevationOffsetProvider):
    ...
    Notes
    -----
    Reads the datatype as signed even if unsigned. Assumes a GDAL GeoTransform
    is provided, which must be independent in x and y (only scale / offset).
    Also assumes single raster.
    """
    ...
    def get_offset(self, geodetic_world_coordinate: GeodeticWorldCoordinate) -> float:
        ...
        if abs(geodetic_world_coordinate.latitude) > np.pi / 2:
            raise ValueError(f"Invalid latitude {geodetic_world_coordinate.latitude}.")
        return self.offset_grid(
            geodetic_world_coordinate.latitude,
            (geodetic_world_coordinate.longitude + np.pi) % (2 * np.pi) - np.pi,
        )[0][0]
